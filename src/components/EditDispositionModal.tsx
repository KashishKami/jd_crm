...
import { DISPOSITION_OPTIONS, CallDispositionRecord } from '../types/callDisposition';
...
export default function EditDispositionModal({
  isOpen,
  dispositionId,
  onClose,
  onSuccess,
  ...
  const fetchDisposition = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/call-dispositions/${dispositionId}`);
      if (!res.ok) throw new Error('Failed to fetch call disposition details');
      const data = await res.json();
      const record: CallDispositionRecord = data.disposition;
      setCustomerPhone(record.customerPhone);
      setCustomerName(record.customerName ?? '');
      setDisposition(record.disposition);
      setErrors({});
    } catch (err: any) {
      setErrors({ global: err.message || 'Failed to load record.' });
    } finally {
      setLoading(false);
    }
  }, [dispositionId]);
  ...
  useEffect(() => {
    if (isOpen && dispositionId) {
      // eslint-disable-next-line react-hooks/set-state-in-effect
      fetchDisposition();
    }
  }, [isOpen, dispositionId, fetchDisposition]);

  if (!isOpen || !mounted || !dispositionId) return null;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(errData.error || 'Failed to update call disposition');
      }
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
          ...
          <p style={{ fontSize: '0.85rem', color: '#64748b', margin: 0 }}>
            Modify the call record details below.
          </p>
        </div>

        {loading ? (
          <div style={{ padding: '24px', textAlign: 'center', color: '#64748b' }}>Loading details...</div>
        ) : (
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px', margin: 0 }}>
            ...
            <div className="form-group" style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              <label htmlFor="edit_customerPhone" className="form-label" style={{ fontSize: '0.875rem', fontWeight: 600, color: '#334155' }}>
                Customer Phone *
              </label>
              <input
                type="text"
            ...
            <div className="form-group" style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              <label htmlFor="edit_customerName" className="form-label" style={{ fontSize: '0.875rem', fontWeight: 600, color: '#334155' }}>
            ...
            <div className="form-group" style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              <label htmlFor="edit_disposition" className="form-label" style={{ fontSize: '0.875rem', fontWeight: 600, color: '#334155' }}>
                Disposition *
              </label>
              <select
                id="edit_disposition"
