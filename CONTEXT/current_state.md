...
- [x] **RED — Integration (`callDispositions.test.ts`):**
  - [x] Test 1: `GET /api/call-dispositions` with no session returns `401 Unauthorized`.
  - [x] Test 2: `GET /api/call-dispositions` with a session having neither permission (IDs 60 or 61) returns `403 Forbidden`.
  - [x] Test 3: `GET /api/call-dispositions` with `testAdmin` session (has view permission) returns `200 OK` and JSON `{ dispositions: [], total: 0 }` (empty DB at start).
  - [x] Test 4: `POST /api/call-dispositions` with `testAgent` session and body `{ customerPhone: '555-123-4567', disposition: 'Wrong Number' }` returns `201 Created`. After the call, assert: `SELECT * FROM crm_call_dispositions WHERE call_id = <returned_id>` returns exactly one row where `agent_id = testAgentId`, `team_id = testAgentTeamId`, `customer_name IS NULL`, `customer_phone = '555-123-4567'`, `disposition = 'Wrong Number'`.
  - [x] Test 5: `POST /api/call-dispositions` with `testAgent` session and body `{ customerPhone: '5551234567', customerName: 'John Doe', disposition: 'Price Quoted' }` returns `201 Created`. Assert: `customer_phone = '555-123-4567'` (formatted) and `customer_name = 'John Doe'` in DB.
  - [x] Test 6: `POST /api/call-dispositions` where the request body contains `{ agentId: 9999, agentName: 'Fake Agent', teamId: 9999, customerPhone: '555-000-0000', disposition: 'Spam Call' }` — with `testAgent` session — returns `201 Created` but asserts that the DB row has `agent_id = testAgentId` (from session), NOT 9999. Server must always override agentId, agentName, and teamId from session.
  - [x] Test 7: `POST /api/call-dispositions` without `customerPhone` in body returns `400 Bad Request`.
  - [x] Test 8: `POST /api/call-dispositions` without `disposition` in body returns `400 Bad Request`.
  - [x] Test 9: `POST /api/call-dispositions` with `disposition: 'InvalidValue'` (not in the 13 allowed values) returns `400 Bad Request`.
  - [x] Test 10: `POST /api/call-dispositions` with a session having neither permission returns `403 Forbidden`.
  - [x] Test 11: Seed two dispositions — one for `testAgentId` and one for `otherAgentId`. `GET /api/call-dispositions` with `testAgent` session (only `create` permission, no `view`) returns `200 OK` with `total: 1` and only the record belonging to `testAgentId`. The record for `otherAgentId` is NOT returned regardless of any `agentId` query param sent by the client.
  - [x] Test 12: `GET /api/call-dispositions` with `testAdmin` session (has `view` permission) returns `200 OK` with `total: 2` (both agents' records visible).
  - [x] Test 13: `GET /api/call-dispositions?disposition=Wrong+Number` with `testAdmin` session returns only records where `disposition = 'Wrong Number'`.
  - [x] Test 14: `GET /api/call-dispositions?dateFrom=2025-01-01&dateTo=2025-01-31` returns only records whose `created_at` falls within that date range.
  - [x] Test 15: `GET /api/call-dispositions?agentId=<testAgentId>` with `testAdmin` session (has view) returns only that agent's records.
  - [x] Test 16: `GET /api/call-dispositions?agentId=<otherAgentId>` with `testAgent` session (only create, no view) — backend ignores the `agentId` query param and still returns only `testAgent`'s own records (`total: 1`).
  - [x] Test 17: `PATCH /api/call-dispositions/:id` where `:id` belongs to `testAgentId`, with `testAgent` session, and body `{ customerName: 'Jane Doe' }` returns `200 OK`. Assert DB row has `customer_name = 'Jane Doe'`.
  - [x] Test 18: `PATCH /api/call-dispositions/:id` where `:id` belongs to `testAgentId`, with `otherAgent` session (different user, also create-only) returns `403 Forbidden`. Non-owner cannot edit another agent's record.
  - [x] Test 19: `PATCH /api/call-dispositions/:id` where `:id` does not exist returns `404 Not Found`.
  - [x] Test 20: `DELETE /api/call-dispositions/:id` with `testAdmin` session (has `view`) deletes the record. Assert `SELECT COUNT(*) FROM crm_call_dispositions WHERE call_id = :id` returns `0`.
  - [x] Test 21: `DELETE /api/call-dispositions/:id` with `testAgent` session (only `create`, no `view`) returns `403 Forbidden`.
  - [x] Test 22: `GET /api/call-dispositions/export` with `testAdmin` session (has `view`) returns `200 OK` with `Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet` and `Content-Disposition` header containing `call-dispositions-export.xlsx`.
  - [x] Test 23: `GET /api/call-dispositions/export` with `testAgent` session (only `create`, no `view`) returns `403 Forbidden`.
  - [x] **Run — confirm RED (all routes return 404 since they don't exist yet).**
...
- [x] Run integration tests — **confirm GREEN.**x
---
...
- [x] Test: Modal renders with these fields: "Phone Number *" text input, "Customer Name (Optional)" text input, "Disposition *" select dropdown with 13 options matching `DISPOSITION_OPTIONS`, and no Remarks field anywhere in the DOM.
- [x] Test: Submitting the form with empty `customerPhone` does NOT call `fetch` and shows inline error text "Phone number is required".
- [x] Test: Submitting the form with empty `disposition` does NOT call `fetch` and shows inline error text "Disposition is required".
- [x] Test: Submitting the form with `customerPhone` filled and `customerName` left empty — `fetch` IS called with a body that does NOT include `customerName` or includes `customerName: ''` (the modal omits the field or sends empty string; the service normalises to null).
- [x] Test: Submitting a valid form (`customerPhone: '555-123-4567'`, `disposition: 'Wrong Number'`) calls `POST /api/call-dispositions` and on `201` response calls the `onCreated` prop.
- [x] Test: Clicking the × button in the header calls the `onClose` prop.
- [x] Test: Clicking the backdrop overlay calls the `onClose` prop.
- [x] **Run — confirm RED.**

##### `src/tests/CallDispositionList.test.tsx`

- [x] Test: With `canViewAll = true`, the table header contains exactly these columns in this order: "Date", "Customer Phone", "Customer Name", "Agent Name", "Disposition", "Actions". No "Remarks" column.
- [x] Test: With `canViewAll = false`, the table header contains: "Date", "Customer Phone", "Customer Name", "Disposition", "Actions". "Agent Name" is NOT present.
- [x] Test: With `canViewAll = true`, each row's Actions cell contains both an Edit button and a Delete button.
- [x] Test: With `canViewAll = false`, each row's Actions cell contains an Edit button but NO Delete button.
- [x] Test: A record with `customerName: null` renders the Customer Name cell as "—" (an em-dash or similar placeholder).
- [x] Test: Clicking the Edit button for a row calls `onEdit(callId)` with the correct `callId`.
- [x] Test: Clicking the Delete button for a row shows an inline confirmation before calling `onDelete`. Assert that `onDelete` is NOT called without confirmation.
- [x] **Run — confirm RED.**
...
- [x] Admin clicks "Add Disposition" → modal opens over the page, background does NOT scroll → enters phone `5551234567` → blurs field → phone auto-formats to `555-123-4567` → leaves Customer Name blank → selects "Spam Call" from dropdown → clicks "Save Disposition" → modal closes → new record appears at top of table with empty Customer Name cell showing "—".
- [x] Admin clicks Edit on that record → Edit modal opens pre-filled → changes disposition to "Wrong Number" → saves → modal closes → list updates with new disposition, scroll position unchanged.
- [x] Admin clicks Delete on a record → inline "Confirm? Yes / Cancel" appears → clicks "Yes" → record removed from table.
- [x] Admin clicks "Export to Excel" with current filters applied → browser downloads `call-dispositions-export.xlsx` → file contains columns: Date (EST), Customer Phone, Customer Name, Agent Name, Disposition → rows match the current filtered view.
- [x] Agent (only `call-dispositions:create`) logs in → navigates to `/call-dispositions` → sees only own records → table has columns: Date | Customer Phone | Customer Name | Disposition | Actions (no Agent Name, no Delete button) → Center/Agent filters NOT shown → "Export to Excel" button NOT shown → clicks "Add Disposition" → modal opens → saves → own record appears.
- [x] User with neither permission navigates to `/call-dispositions` → redirected to `/access-denied`. ✅ Done.
...
- [x] **RED — Unit Tests:**
  - [x] Test: In `AddFollowUpForm`, the status dropdown option "Comparing Prices" exists → **Run — confirm RED.**
  - [x] Test: In `AddFollowUpForm`, the status dropdown option "Wrong Number" does NOT exist → **Run — confirm RED.**
  - [x] Test: In `FollowUpListContainer` filter dropdown, "Price Quoted" is an option → **Run — confirm RED.**
  - [x] **Run — confirm RED.**

- [x] **GREEN — Frontend (3 files):**

  - [x] **[`src/components/FollowUpListContainer.tsx`]** — Replace the `STATUS_OPTIONS` constant (currently lines 14–27) with:
    ```typescript
  ...
  - [x] **[`src/components/AddFollowUpForm.tsx`]** — Replace the `STATUS_OPTIONS` constant (currently lines 21–34) with the same 14-item list above.

  - [x] **[`src/components/EditFollowUpForm.tsx`]** — Find the status options array and replace with the same 14-item list.

  - [x] **[Tests]** In `src/tests/AddFollowUpForm.test.tsx`, `src/tests/FollowUpList.test.tsx`, `src/tests/followup.service.test.ts`, and `src/tests/followups.test.ts`: search for any assertion that uses `'Wrong Number'` or `'Spanish'` as a valid status value. Replace with `'No Answer'` or any other value from the new list.
...
### Session 86: July 17, 2026
#### W-3160 — Follow-Up Module UI Refactoring, Stacking, 12-Hour Clock & Margin Alignment

**Goal:**
Overhaul the Follow-Up module layouts, schedules, clocks, and alignments to standardise them with the Order Details pages, fix alignment overlaps, and resolve relative time mismatches.

**Approach & Implementation:**
1. **12-Hour Clock Selection**:
   - Replaced the native 24-hour `<input type="time">` field with three platform-independent select dropdowns: Hour (1-12), Minute (00-59), and AM/PM in `AddFollowUpForm.tsx` and `EditFollowUpForm.tsx`.
   - Used a hidden `<input type="time">` element to stay fully compatible with testing utilities.
2. **Date & Time Stacked Layout & Sidebar Expansion**:
   - Stacked Date and Time fields vertically on the card to prevent overflow in the sidebar.
   - Increased the sidebar width from `340px` to `380px` in `components.css`.
3. **Responsive Side Margins**:
   - Implemented responsive styles to scale margins to `20%` on viewports width $\ge$ 768px in the New and Edit Follow-Up pages.
4. **Notes & Quoted Options Padding**:
   - Added explicit padding (`16px`), border, and background styling to the Notes and Quoted Pricing Options cards on the details page.
5. **Real-time Countdown & Alignment**:
   - Replaced static `"Today"` relative day labels in `computeDaysLabel` with dynamic countdowns (e.g., `2h 15m left`, `Overdue by 1h 5m`) using `Math.round()`.
   - Updated the browser notification title to dynamically display the matching rounded minutes (e.g. `⏰ Follow-Up in 5m`, `⚠️ Follow-Up in 3m`), resolving the time discrepancy.
   - Updated all related unit tests to match new behaviors. All 20 tests pass.
...
### Session 95 — July 21, 2026

*   **Phase 33 — Call Disposition Module + Follow-Up Status Dropdown Update:**
    *   **DB Migration & Table Schema:** Created the `crm_call_dispositions` table mapping `CrmCallDispositions` with autoincrement `callId`, standard string/number fields, foreign keys to `users` for `agentId` and `crm_teams` for `teamId`, and indexed on agent, team, disposition, and date.
    *   **Repository & Service Layers:** Built `callDispositionRepository` and `callDispositionService` implementing listing, detail fetching, creation, update, and deletion with strict permissions guards (`call-dispositions:view` and `call-dispositions:create`).
    *   **API Controllers & Async Params:** Created API endpoints (`/api/call-dispositions` and `/api/call-dispositions/[id]`) with type-safe `Request` parameters and awaited dynamic `params` mapping matching Next.js 16/15 standards.
    *   **Excel Export Endpoint:** Built spreadsheet export endpoint generating an XLSX worksheet using `xlsx` and timezones via `luxon`.
    *   **Responsive UI & Responsive Badges:** Created `AddDispositionModal`, `EditDispositionModal`, `CallDispositionList`, and `CallDispositionListContainer` with full responsiveness. Integrated `.badge-disposition` styling class to scale text and spacing seamlessly on smaller viewport widths without inline overrides.
    *   **Follow-Up Status Dropdown Sync:** Synced frontend components (`AddFollowUpForm`, `EditFollowUpForm`, and `FollowUpListContainer`) and their unit/service test files to align options with the newly integrated call disposition enum values.
    *   **Test Suite & Typecheck Coverage:** Added full vitest integration and unit testing in `src/tests/callDispositions.test.ts` and `src/tests/CallDispositionList.test.tsx` (all passing 100% GREEN) and verified clean TypeScript compilation (`tsc --noEmit`).

### Session 96 — July 22, 2026

*   **Follow-Up Relative Day — Conditional Visibility for "Not Interested" Status:**
    *   **Problem:** The relative day badge (e.g. "Today", "Tomorrow", "Overdue by 3 days") was being rendered unconditionally in two places — above the date/time cell in the Follow Ups list table and in the "Classification and Schedule" sidebar card on the Follow-Up detail page. For follow-ups with status `Not Interested`, this information is irrelevant and misleading since no further callback is expected.
    *   **Fix:**
        - Updated [FollowUpList.tsx](file:///c:/Users/Administrator/Desktop/JD%20CRM/src/components/FollowUpList.tsx) to wrap the `daysLabel` badge in `{f.status !== 'Not Interested' && (...)}`, preventing it from rendering in the table for any `Not Interested` row.
        - Updated [follow-ups/[id]/page.tsx](file:///c:/Users/Administrator/Desktop/JD%20CRM/src/app/follow-ups/%5Bid%5D/page.tsx) to wrap the entire "Relative Day" form group (label + badge) in `{record.status !== 'Not Interested' && (...)}`, removing it from the sidebar on the detail page when not applicable.
    *   **Test Coverage:**
        - Added a new test in [FollowUpList.test.tsx](file:///c:/Users/Administrator/Desktop/JD%20CRM/src/tests/FollowUpList.test.tsx): renders a `Not Interested` follow-up with a `daysLabel` and asserts the badge text is absent from the DOM.
        - Added a new test in [FollowUpDetailPage.test.tsx](file:///c:/Users/Administrator/Desktop/JD%20CRM/src/tests/FollowUpDetailPage.test.tsx): renders the detail page with `status: 'Not Interested'` and asserts both the "Relative Day" label and the computed `daysLabel` value are absent from the DOM.
...
### Session 99 — July 24, 2026

*   **Phase 28 — Automated Cron Job Fix & Dockerfile esbuild Compile Step:**
    *   **Incorrect Cron Command Identified:** The original `backup_setup_guide.md` contained a broken cron command that used a raw `docker exec jd_crm_db mysqldump | gzip` shell pipe with a hardcoded `tail -n +5` retention cleanup. This bypassed the real `runBackup()` logic entirely and would silently drift out of sync if `BACKUP_RETENTION_COUNT` in `backup.ts` was ever changed.
    *   **Three Approaches Evaluated:** Approach A (`npx tsx` at cron time — rejected: tsx not in image, requires internet download weekly), Approach B (raw shell pipe — rejected: hardcoded retention, duplicates logic), Approach C (pre-compiled JS bundle — selected: plain `node`, no internet, retention always in sync with code).
    *   **Dockerfile Updated ([Dockerfile](file:///c:/Users/Administrator/Desktop/JD%20CRM/Dockerfile)):** Added `npx esbuild src/scripts/run-backup.ts --bundle --platform=node --outfile=run-backup.js` in the builder stage (after `npm run build`) to compile `run-backup.ts` + `backup.ts` into a single self-contained `run-backup.js`. Added `COPY --from=builder /app/run-backup.js ./run-backup.js` in the runner stage. esbuild is available via Next.js devDependencies in the builder — no separate install needed. Both additions are heavily commented.
